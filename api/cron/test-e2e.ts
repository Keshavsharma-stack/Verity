...
import { processWorkspaceExpirationScan, getSupabaseUserClient } from '../../src/server/notificationLogic.js';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // If a valid SUPABASE_SERVICE_ROLE_KEY is provided in the environment, use a service client.
    // Otherwise, use the user client (supabase) which carries the authenticated user's JWT and passes workspace RLS.
...
%%EOF`;
    ...
    // Strategy 1: Standard File instance upload with explicit headers
    try {
    ...
    // Strategy 3: Buffer direct fallback
    if (!uploadSuccess) {
      try {
    ...
    if (!uploadSuccess) {
      ...
      // Rollback contractor and requirements before returning error
      try {
    ...
    if (primaryInsertRes.error || !primaryInsertRes.data) {
      ...
      // Clean up storage file, contractor and requirements before returning
      try {
