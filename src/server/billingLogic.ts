...
export async function handleGetSubscription(req: any, res: any) {
  ...
  try {
    ...
    // Proactively verify & sync status if Cashfree subscription exists or cf_sub_id query parameter is present
    const appId = process.env.CASHFREE_APP_ID;
    const secretKey = process.env.CASHFREE_SECRET_KEY;
    const cashfreeSubId = (req.query?.cf_sub_id as string) || subData.cashfree_subscription_id;

    if (cashfreeSubId && appId && secretKey) {
      try {
        const cfFetchRes = await fetch(`${getCashfreeBaseUrl()}/subscriptions/${cashfreeSubId}`, {
          headers: {
            'X-Client-Id': appId,
        ...
        if (cfFetchRes.ok) {
          const cfSubDetails: any = await cfFetchRes.json();
          const cfStatus = cfSubDetails.subscription_status || cfSubDetails.status;
          const cfPlanId = cfSubDetails.plan_details?.plan_id || cfSubDetails.plan_id;

          if (cfStatus === 'ACTIVE' || cfStatus === 'COMPLETED') {
            let planSlug = 'STARTER';
            if (cfPlanId && String(cfPlanId).toLowerCase().includes('pro')) {
              planSlug = 'PRO';
            }
            ...
            await adminClient
              ...
              .upsert({
                ...
                plan: planSlug,
                status: 'active',
                cashfree_subscription_id: cashfreeSubId,
                cashfree_plan_id: cfPlanId || null,
          ...
          } else if (['CUSTOMER_CANCELLED', 'EXPIRED', 'CANCELLED'].includes(cfStatus)) {
            await adminClient
              .from('subscriptions')
              .update({
                plan: 'FREE',
      ...
      } catch (cfFetchErr: any) {
        console.warn(`[Billing [${reqId}]] Live Cashfree sync check warning:`, cfFetchErr?.message);
      }
    }
...
// -----------------------------------------------------------------------------
// POST /api/billing/checkout (Replaced with Cashfree Web Subscriptions Checkout)
// -----------------------------------------------------------------------------
export async function handleCheckout(req: any, res: any) {
  ...
  try {
    ...
    const subscriptionSessionId =
      cfData.subscription_session_id ||
      cfData.subscriptionSessionId ||
      cfData.sub_session_id ||
      cfData.subsSessionId ||
      cfData.session_id ||
      null;

    const authLink = cfData.auth_link || cfData.authLink || null;
    const cashfreeEnv =
      (process.env.CASHFREE_ENV || 'SANDBOX').toUpperCase() === 'PROD' ||
      (process.env.CASHFREE_ENV || 'SANDBOX').toUpperCase() === 'PRODUCTION'
        ? 'production'
        : 'sandbox';

    if (!subscriptionSessionId && !authLink) {
      console.error(`[Checkout [${reqId}]] Cashfree response missing session ID:`, cfData);
      return res.status(500).json({
        error: cfData.message || 'Cashfree did not return a valid subscription session ID',
        reqId
      });
    }
